...
columns = ["Molecular_weight", "LogP", "Number_of_HBA_1", "Number_of_HBA_2",
...
def main(): 
	...
	for mol in pybel.readfile('sdf', inputTempName):
		...
		cmd = 'JAVA_HOME=/opt/jre/ JOELIBDIR=/opt/JOELib/ CLASSPATH=/opt/JOELib/build /opt/JOELib/convert.sh +d -iSDF -oSDF ' + sdfName + ' '+fh.name+' > /dev/null'
		os.system(cmd)
		os.unlink(sdfName)
		info = getinfo(fh)
