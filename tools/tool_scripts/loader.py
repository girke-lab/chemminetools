...
parser.add_argument('-i','--infile', help='input YAML file', required=True)
...
def main():
	...
	f.close()
	try:
		category = ApplicationCategories.objects.get(name=data['category'])
	except:
	...
	newApp = Application(name=data['name'], script=data['script'],input_type=data['input_type'],output_type=data['output_type'], description=data['description'], category=category)
	newApp.save()	
	try:
		for option in data['ApplicationOptions'].keys():
			optionData = data['ApplicationOptions'][option]
			newOption = ApplicationOptions(name=option, realName=optionData['realName'], application=newApp)
			newOption.save()
			for optionValue in optionData['options']:	
				newValue = ApplicationOptionsList(category=newOption, name=optionValue[0], realName=optionValue[1])
				newValue.save()
	except:
		pass
