...
def search(request):
	if request.method == 'GET':
		...
		if not sdf.strip():
			...
			if os.path.exists(query_sdf): os.unlink(query_sdf)
			f = file(query_sdf, 'w')
			f.write(sdf)
			f.close()
...
def session_check(f):
	def func(request, sid, *args, **kargs):
		...
		if os.path.exists(status_fp):
			...
			status_f.close()
		if not os.path.exists(status_fp) or (status != 'done' and status != 'failed'):
			if time() - os.stat(query)[ST_MTIME] > settings.QUERY_TIMEOUT:
				...
				if 'ajax' in request.GET:
					return HttpResponse('{"status":"wait"}', mimetype='text/plain')
				return HttpResponseRedirect(reverse('search_wait', kwargs=dict(sid=sid)))
		...
		if status == 'failed':	
			...
			return render_to_response(
				'eis/result.html',
				dict(sid=sid, mode='error', error=error),
				context_instance=RequestContext(request)
			)
...
def result(request, sid):
	...
	return render_to_response(
		'eis/result.html',
		dict(sid=sid, mode='ready', timing=timing, compounds=compounds,
			r_timing=r_timing, t_table_width=t_table_width,
			total_t=total_t, slow=(total_t > 1.5)),
		context_instance=RequestContext(request)
	)
...
def download(request, sid, format):
	...
	fo.write(cids_str)
	fo.close()
	cmd = '%s %s %s %s' % (settings.PUBCHEM_DOWNLOADER, tmp_fp, fp, format)
	Popen(cmd, close_fds=True, shell=True)

	return render_to_response(
		'compound_download.html',
		dict(sid=s),
		context_instance=RequestContext(request)
		)
