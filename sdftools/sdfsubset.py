...
if len(sys.argv) != 4:
	...
	sys.stderr.write('If second arguments ends in .sdf, a single file will '
			'be created. Otherwise, a\n'
			'folder will be created and each SDF will be placed in a '
			'separate file\n')
	sys.exit(1)
inp = sys.argv[1]
...
for i in iter:
	cntr += 1
	if not cntr in indices: continue
	if mode == 'single-file':
		outf.write(i)
	elif mode == 'directory':
		#name = i.split('\n')[0].strip()
