...
inputConverters = {
    ...
    'application/json.graph': 'igraph\ntoJSON(list(nodes=lapply(V(input)$name, function(x){list(data=list(id=x))}),edges=mapply(function(x,y){list(data= list(source= x, target= y))}, get.edgelist(input)[,1], get.edgelist(input)[,2], USE.NAMES=FALSE)))'
}
